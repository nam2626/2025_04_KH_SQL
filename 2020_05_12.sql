...
WHERE MNAME LIKE '%공학%';
UPDATE STUDENT SET MNAME = REPLACE(MNAME, '공학', '학')
WHERE INSTR(MNAME, '공학') <> 0;

SELECT * FROM STUDENT;

-- LPAD, RPAD : 원하는 문자열 개수만큼 남은 부분에 지정한 문자열로 채우는 함수
SELECT LPAD('991122-1', 14, '*') FROM DUAL;
SELECT RPAD('991122-1', 14, '*') FROM DUAL;
-- 1234123ABC ABC1234123
...
SELECT LTRIM('AAAAABBBBBBCCCCCDDDDAAAAA','A') FROM DUAL;
SELECT RTRIM('AAAAABBBBBBCCCCCDDDDAAAAA','A') FROM DUAL;
...
-- 연락처 : 010 1111 2222
SELECT '010' || '1111' || '2222' FROM DUAL;
SELECT CONCAT('010',CONCAT('1111','2222')) FROM DUAL;
...
-- 2. RPAD, LENGTH
SELECT RPAD(SUBSTR('841113-1246121',1,8),
        LENGTH('841113-1246121'),'*') FROM DUAL;
