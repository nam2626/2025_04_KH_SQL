...
FROM STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO
WHERE S.SNO IN(SELECT SS.SNO FROM STUDENT_SCHOLARSHIP SS);
--장학금을 받는 못하는 학생들만조회			
SELECT S.SNO, S.SNAME, M.MNAME, S.SCORE
FROM STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO
WHERE S.SNO NOT IN(SELECT SS.SNO FROM STUDENT_SCHOLARSHIP SS);
-- 학과별로 최고점을 가진 학생들을 조회
-- 학번, 이름, 학과명, 평점
SELECT MNO, MAX(SCORE) FROM STUDENT GROUP BY MNO;
SELECT DISTINCT MNO, MAX(SCORE) OVER(PARTITION BY MNO) FROM STUDENT;

SELECT S.SNO, S.SNAME, M.MNAME, S.SCORE
FROM STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO
WHERE (S.MNO, S.SCORE) IN(SELECT DISTINCT MNO, MAX(SCORE) OVER(PARTITION BY MNO) FROM STUDENT);

SELECT S.SNO, S.SNAME, M.MNAME, S.SCORE
FROM STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO
WHERE (S.MNO, S.SCORE) IN(SELECT MNO, MAX(SCORE) FROM STUDENT GROUP BY MNO);

-- FROM 절에 들어가는 서브쿼리(인라인 뷰)
--  학생정보 조회시, 행번호, 학번, 이름, 학과명, 평점을 조회
--  평점을 기준으로 내림차순 정렬을 해서 조회, 행번호는 ROWNUM
SELECT 
    ROWNUM, S.*
FROM
    (SELECT S.SNO, S.SNAME, M.MNAME, S.SCORE
    FROM STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO
    ORDER BY S.SCORE DESC) S;

SELECT 
    ROW_NUMBER() OVER(ORDER BY S.SCORE DESC) ,S.SNO, 
    S.SNAME, M.MNAME, S.SCORE
FROM 
    STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO;
...
SELECT * 
FROM
    (SELECT ROW_NUMBER() OVER (ORDER BY S.SNO) AS RW, 
        S.SNO, S.SNAME, M.MNAME, S.SCORE
    FROM STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO)
WHERE RW BETWEEN 6 AND 10;
...
SELECT 
    S.SNO, S.SNAME, 
    (SELECT M.MNAME FROM MAJOR M WHERE M.MNO = S.MNO) AS MNAME, 
    S.SCORE, S.GENDER
FROM STUDENT S;

-- WITH
WITH AVG_SCORE AS (
    SELECT AVG(SCORE) AS VALUE FROM STUDENT
)
SELECT * FROM STUDENT 
WHERE SCORE >= (SELECT VALUE FROM AVG_SCORE);

WITH STUDENT_MAJOR AS (
    SELECT * FROM STUDENT NATURAL JOIN MAJOR
)
SELECT *
FROM STUDENT_SCHOLARSHIP SS INNER JOIN STUDENT_MAJOR SM ON SS.SNO = SM.SNO; 
...
WHERE SRANK <= 3;
-- 3위까지 학생이거나 평균 이하인 학생을 조회 + 전공 이름 + 순위 별로 정렬 적용
-- 서브 쿼리 및 WITH 적용
WITH STUDENT_RANK AS (
    SELECT DENSE_RANK() OVER (ORDER BY S.SCORE DESC) AS SRANK,
    S.SNO, S.SNAME, M.MNAME, S.SCORE, S.GENDER
...
-- 성별, 성별 평균, 전체 평균
-- 그룹
SELECT GENDER, AVG(SCORE) AS GENDER_SCORE FROM STUDENT GROUP BY GENDER;
-- 윈도우
SELECT DISTINCT GENDER, AVG(SCORE) OVER(PARTITION BY GENDER) AS GENDER_SCORE 
FROM STUDENT;
-- 전체 평균
SELECT AVG(SCORE) AS ALL_AVG_SCORE FROM STUDENT;
...
SELECT GA.*, (SELECT * FROM ALL_AVG) AS ALL_AVG_SCORE FROM GENDER_AVG GA;
...
SELECT * FROM GENDER_AVG GA CROSS JOIN ALL_AVG AA;


-- 전공별 평균보다 높은 점수를 받은 학생을 조회
-- 학번, 이름, 학과명, 점수, 전공별 평균
-- 학과번호, 학과 평점 평균
SELECT MNO, AVG(SCORE) AS AVG_SCORE
    FROM STUDENT GROUP BY MNO;

WITH MAJOR_AVG AS (
    SELECT MNO, AVG(SCORE) AS AVG_SCORE
    FROM STUDENT GROUP BY MNO
)
SELECT S.SNO, S.SNAME, M.MNAME, S.SCORE, MA.AVG_SCORE
FROM STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO
INNER JOIN MAJOR_AVG MA ON S.MNO = MA.MNO
WHERE S.SCORE > MA.AVG_SCORE;

-- 전공별 최고 점수자 조회
-- 학번, 이름, 학과명, 점수
-- case 1
-- 전공별 최대 점수 조회
SELECT MNO, MAX(SCORE) AS MAX_SCORE FROM STUDENT GROUP BY MNO;

WITH MAX_SCORE_STUDENTS 
AS (SELECT MNO, MAX(SCORE) AS MAX_SCORE FROM STUDENT GROUP BY MNO)
SELECT S.SNO, S.SNAME, M.MNAME, S.SCORE
FROM STUDENT S INNER JOIN MAJOR M ON S.MNO = M.MNO
WHERE (S.MNO, S.SCORE) IN(SELECT * FROM MAX_SCORE_STUDENTS);

-- 전공별 점수 순위 조회
SELECT RANK() OVER(PARTITION BY MNO ORDER BY SCORE DESC  ) AS RANK_SCORE,
S.* FROM STUDENT S;

WITH RANK_STUDENTS AS 
(SELECT RANK() OVER(PARTITION BY MNO ORDER BY S.SCORE DESC  ) AS RANK_SCORE,
S.* FROM STUDENT S)
SELECT RS.SNO, RS.SNAME, M.MNAME, RS.SCORE 
FROM RANK_STUDENTS RS INNER JOIN MAJOR M ON RS.MNO = M.MNO
WHERE RS.RANK_SCORE = 1; 

-- EXISTS : 행이 존재 하는가?
-- 장학금을 받는 학생들을 서브쿼리로 조회
SELECT * FROM STUDENT S
WHERE S.SNO IN(SELECT SS.SNO FROM STUDENT_SCHOLARSHIP SS);

SELECT * FROM STUDENT S
WHERE EXISTS(
    SELECT 1 FROM STUDENT_SCHOLARSHIP SS WHERE SS.SNO = S.SNO
);

-- 장학금을 받지 않는 학생들을 서브쿼리로 조회
SELECT * FROM STUDENT S
WHERE S.SNO NOT IN(SELECT SS.SNO FROM STUDENT_SCHOLARSHIP SS);
