...
SELECT TO_CHAR(CS.CAR_SELL_DATE,'YYYY/MM') AS SELL_YEAR, CM.CM_NAME, 
    SUM(CS.CAR_SELL_EA) AS TOTAL_EA,
    SUM(CS.CAR_SELL_PRICE) AS TOTAL_PRICE,
    CEIL(AVG(CS.CAR_SELL_PRICE)) AS AVG_PRICE
FROM CAR C INNER JOIN CAR_MAKER CM ON C.CM_ID = CM.CM_ID
INNER JOIN CAR_SELL CS ON C.ID = CS.ID
GROUP BY TO_CHAR(CS.CAR_SELL_DATE,'YYYY/MM'), CM.CM_NAME;

--판매 연도/분기, 판매 대수 총합, 판매금액 총합, 판매금액 평균을 조회
SELECT TO_CHAR(CS.CAR_SELL_DATE,'YYYY"년도" Q"분기"') AS SELL_YEAR, CM.CM_NAME, 
    SUM(CS.CAR_SELL_EA) AS TOTAL_EA,
    SUM(CS.CAR_SELL_PRICE) AS TOTAL_PRICE,
    CEIL(AVG(CS.CAR_SELL_PRICE)) AS AVG_PRICE
FROM CAR C INNER JOIN CAR_MAKER CM ON C.CM_ID = CM.CM_ID
INNER JOIN CAR_SELL CS ON C.ID = CS.ID
GROUP BY TO_CHAR(CS.CAR_SELL_DATE,'YYYY"년도" Q"분기"'), CM.CM_NAME;
