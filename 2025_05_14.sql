...
-- 3. 직급별 평균 급여를 조회하세요.
SELECT POSITION, FLOOR(AVG(SALARY)) AS EMP_POS_COUNT 
FROM EMPLOYEE GROUP BY POSITION;
SELECT 
    DISTINCT POSITION, 
    FLOOR(AVG(SALARY) OVER(PARTITION BY POSITION)) 
FROM EMPLOYEE;
-- 4. 부서별 최대 급여와 최소 급여를 조회하세요.
SELECT DNAME, MAX(SALARY), MIN(SALARY) FROM EMPLOYEE GROUP BY DNAME;
SELECT DISTINCT DNAME, 
    MAX(SALARY) OVER(PARTITION BY DNAME), 
    MIN(SALARY) OVER(PARTITION BY DNAME)
FROM EMPLOYEE;
-- 5. 부서별 사원의 총 급여 합계를 조회하세요.
SELECT DNAME, SUM(SALARY) 
FROM EMPLOYEE WHERE POSITION='사원'
GROUP BY DNAME;
...
FROM EMPLOYEE
GROUP BY POSITION HAVING COUNT(*) >= 20;
-- 7. 부서별 평균 급여가 3,000,000 이상인 부서만 조회하세요.
SELECT DNAME, AVG(SALARY) FROM EMPLOYEE
GROUP BY DNAME HAVING AVG(SALARY) >= 3000000;
-- 8. 직급별 평균 급여를 내림차순으로 정렬해서 출력하세요.
SELECT POSITION, AVG(SALARY) AS EMP_AVG_SALARY FROM EMPLOYEE
GROUP BY POSITION HAVING AVG(SALARY) >= 3000000
ORDER BY EMP_AVG_SALARY DESC;
-- 9. 부서별 급여 평균과 급여 총합을 함께 출력하세요.
SELECT DNAME, AVG(SALARY), SUM(SALARY) FROM EMPLOYEE
GROUP BY DNAME;
-- 10. 입사 연도별 사원 수를 조회하세요. (SUBSTR(HIREDATE, 1, 4) 사용)
SELECT TO_CHAR(HIREDATE,'YYYY'), COUNT(*) AS CNT
FROM EMPLOYEE
GROUP BY TO_CHAR(HIREDATE,'YYYY')
ORDER BY CNT;

SELECT DISTINCT TO_CHAR(HIREDATE,'YYYY'), 
    COUNT(*) OVER(PARTITION BY TO_CHAR(HIREDATE,'YYYY')) AS CNT
FROM EMPLOYEE
ORDER BY CNT;

-- 1. 전체 사원의 급여 순위를 조회하세요. (RANK())
SELECT RANK() OVER(ORDER BY SALARY DESC), E.* FROM EMPLOYEE E;
-- 2. 동일 급여에 대해 순위 건너뛰지 않고 부여하세요. (DENSE_RANK())
SELECT DENSE_RANK() OVER(ORDER BY SALARY DESC), E.* FROM EMPLOYEE E;
-- 3. 급여가 높은 순으로 행 번호를 부여하세요. (ROW_NUMBER())
SELECT ROW_NUMBER() OVER(ORDER BY SALARY DESC), E.* FROM EMPLOYEE E;
-- 4. 급여가 높은 순서에서 다음 사원의 이름을 출력하세요. (LEAD())
-- 5. 급여가 높은 순서에서 이전 사원의 이름을 출력하세요. (LAG())
SELECT 
LEAD(ENAME) OVER(ORDER BY SALARY DESC), 
LAG(ENAME) OVER(ORDER BY SALARY DESC), 
E.* 
FROM EMPLOYEE E;